...
class Campus_Config:
    ...
    def campus_underlay(self, spine_ips, leaf_ips, server_ips, campus_info, server_hostip_map):
	helpers = Helpers()
	    ...
	    on_box_hostname = dev.facts["hostname"]
	        ...
	        remote_system = lldp.findtext("lldp-remote-system-name")
	            ...
	            ints_list.append({'physical_interface': local_port, 'description': description, 'ip_address': "None" })
                    ...
                    leaf_links_list.append({'ae_id':ae_list_id,'local_system': on_box_hostname, 'local_port': local_port, 'remote_system': remote_system, 'remote_port': remote_port})
